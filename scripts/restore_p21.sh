...
if [ $? -eq 0 ] ; then
   ...
   sleep 5 | tee >(zenity --progress --pulsate --no-cancel --auto-close --title="Information" \
   --text="\n\n  Testing the Internet over TOR was successful !  \n\n" > /dev/null 2>&1)
else
   if [ $CLI_OUT == "1" ] ; then
